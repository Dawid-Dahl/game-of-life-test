...
var main = function main() {
  ...
  var nextGeneration = function nextGeneration(grid) {
    var nextGenGrid = grid.map(function (col) {
      return _toConsumableArray(col);
    });
        ...
        for (var i = -1; i < 2; i++) {
          for (var j = -1; j < 2; j++) {
            ...
            if (xcell >= 0 && ycell >= 0 && xcell < cols && ycell < rows) {
              var neighbour = grid[col + i][row + j];
  ...
  var buildGrid = function buildGrid(cols, rows) {
    return new Array(cols).fill(null).map(function (col) {
      return new Array(rows).fill(0);
    });
  };

  var initialGrid = buildGrid(cols, rows);

  var randomizeGrid = function randomizeGrid(grid) {
  ...
  var grid = randomizeGrid(initialGrid);
